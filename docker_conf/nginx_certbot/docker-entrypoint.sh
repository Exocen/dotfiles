...
certbot_renew(){
    ...
    /usr/bin/certbot renew --nginx --logs-dir /dev/null
...
RESULT=$?
if [ $RESULT -eq 1 ]; then
    /usr/bin/certbot --nginx --keep-until-expiring --logs-dir /dev/null --expand --register-unsafely-without-email --agree-tos -d [DOMAIN] -d vw.[DOMAIN] -d www.[DOMAIN] || (echo "certbot failed exiting..." && exit 1)
fi
