...
function create() {
	...
	printf "#!/bin/sh\n/root/install.sh -n -l /dev/stdout" "$img_name" >"$tmpD"/test-engine.sh
	cd "$tmpD" || exit 1
	...
	logs["$img_name"]="$(docker run --rm --name=cont_"$img_name" "$img_name"_img)" &&
		echo "$img" started
	rm -r "$tmpD"
}
...
for img3 in "${imgs[@]}"; do
	...
	if echo "${logs["$img_name"]}" | tail -n 1 | grep "\[success\] Installation successful" &>/dev/null; then
