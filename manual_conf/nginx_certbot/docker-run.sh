...
docker build --build-arg DOMAIN=$1 -t nginx_certbot_img . && \
    \
    docker run -v /etc/letsencrypt/:/etc/letsencrypt/ -v /var/log/nginx_certbot:/var/log -p 80:80 -p 443:443 --name nginx_certbot -d --rm  nginx_certbot_img:latest &&\
    docker exec -it nginx_certbot bash
